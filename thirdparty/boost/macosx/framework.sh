...
rm -rf "${BUILD_ROOT}"
mkdir -p "${BUILD_ROOT}"
lipo -create "${PLATFORM_ROOT}"/build-{x86_64,arm64}/stage/libboost.a -output "${BUILD_ROOT}/libboost.a"

rm -rf "${FRAMEWORK_ROOT}"
xcodebuild -create-xcframework -library "${BUILD_ROOT}/libboost.a" -headers "${PLATFORM_ROOT}/build-x86_64/stage/include" -output "${FRAMEWORK_ROOT}"
